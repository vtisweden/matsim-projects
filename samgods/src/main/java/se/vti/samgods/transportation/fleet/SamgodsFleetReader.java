	...
	public void load_v12(String vehicleTypeFile, String costFile, SamgodsConstants.TransportMode transportMode)
		...
		for (CSVRecord record : CSVFormat.EXCEL.withFirstRecordAsHeader().parse(new FileReader(vehicleTypeFile))) {
			for (boolean container : new boolean[] { false, true }) {
				...
				builder.setName(record.get(VEH_LABEL) + (container ? SUFFIX_INDICATING_CONTAINER : ""))
						...
						.setCost_1_km(parseDouble(record.get(COST_1_KM))).setCost_1_h(parseDouble(record.get(COST_1_H)))
						.setCapacity_ton(parseDouble(record.get(CAPACITY_TON)))
						.setOnFerryCost_1_km(parseDouble(record.get(ON_FERRY_COST_1_KM)))
						.setOnFerryCost_1_h(parseDouble(record.get(ON_FERRY_COST_1_H)))
						.setSpeed_km_h(parseDouble(record.get(SPEED_KM_H)));
			}
		}
		...
		for (CSVRecord record : CSVFormat.EXCEL.withFirstRecordAsHeader().parse(new FileReader(costFile))) {
			final SamgodsConstants.Commodity commodity = SamgodsConstants.Commodity
					.values()[parseInt(record.get(COMMODITY_ID)) - 1];
			vehicleNr2builder.get(record.get(VEH_NR)).setContainer(false)
					.setLoadCost_1_ton(commodity, parseDouble(record.get(NO_CONTAINER_LOAD_COST_1_TON)))
					.setLoadTime_h(commodity, parseDouble(record.get(NO_CONTAINER_LOAD_TIME_H)))
					.setTransferCost_1_ton(commodity, parseDouble(record.get(NO_CONTAINER_TRANSFER_COST_1_TON)))
					.setTransferTime_h(commodity, parseDouble(record.get(NO_CONTAINER_TRANSFER_TIME_H)));
			vehicleNr2builder.get(record.get(VEH_NR) + SUFFIX_INDICATING_CONTAINER).setContainer(true)
					.setLoadCost_1_ton(commodity, parseDouble(record.get(CONTAINER_LOAD_COST_1_TON)))
					.setLoadTime_h(commodity, parseDouble(record.get(CONTAINER_LOAD_TIME_H)))
					.setTransferCost_1_ton(commodity, parseDouble(record.get(CONTAINER_TRANSFER_COST_1_TON)))
					.setTransferTime_h(commodity, parseDouble(record.get(CONTAINER_TRANSFER_TIME_H)));
		}
